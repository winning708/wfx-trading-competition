# MT5 REST API Integration Guide

## Overview

The MT5 REST API integration allows you to automatically sync trading account data from MetaTrader 5 platforms to the WFX Trading Competition leaderboard. This provides direct access to real-time account performance metrics.
...
MT5 Account
       ↓
fetchMT5AccountData() [server/lib/mt5-client.ts]
       ↓
syncMT5Account() returns {balance, profit%}
       ↓
...
1. **Trading Account**: MT5 account with JustMarkets or another broker
2. **API Access**: One of:
   - Broker's native REST API endpoint
   - MetaApi (metaapi.cloud) - professional MT5 REST API service
   - Custom MT5 WebSocket-to-REST bridge
3. **API Credentials**:
...
MetaApi provides a professional REST API for MT5 accounts:

1. Go to https://metaapi.cloud
2. Sign up or log in
3. Connect your JustMarkets MT5 account
4. Get your Account ID from MetaApi dashboard
5. Generate an API token from Settings → API Tokens
6. Your server endpoint will be: `https://mt5api.metaapi.cloud` (or the MetaApi endpoint they provide)
...
If using a custom MT5 REST bridge:
...
3. Fill in the form:
   ...
   - **MT5 Account ID**: Your trading account number (e.g., 1234567)
   - **API Token/Password**: Your API credential
   - **Server Endpoint**: REST API URL (e.g., https://mt5api.metaapi.cloud)
...
**Single Integration:**
```bash
curl -X POST http://localhost:3000/api/sync/mt5/trigger/{integrationId}
```

**All Integrations:**
```bash
curl -X POST http://localhost:3000/api/sync/mt5/trigger
```
...
import { handleMT5SyncAll } from './routes/sync';

// Schedule sync every 15 minutes
cron.schedule('*/15 * * * *', async () => {
  console.log('[Cron] Running scheduled MT5 sync...');
...
{
  "success": true,
  "message": "MT5 Sync complete: 2 success, 0 failed",
  "synced": 2,
  "failed": 0
}
```
...
{
  "success": false,
  "message": "MT5 Sync failed: Failed to fetch account data from MT5"
}
```

## Supported Account Data

The MT5 client fetches:
...
### "Failed to fetch account data from MT5"
...
4. Check if the MT5 account is still active on the broker
...
- **`server/lib/mt5-client.ts`**: MT5 API communication
  - `fetchMT5AccountData()` - Fetch account data from API
  - `syncMT5Account()` - Full sync workflow
  - `testMT5Connection()` - Test API connection

- **`server/lib/supabase-client.ts`**: Database operations (new functions)
  - `getActiveMT5Integrations()` - Get all active MT5 integrations
  - `createMT5Integration()` - Create new integration
  - `updateMT5IntegrationSyncStatus()` - Update sync status

- **`server/routes/sync.ts`**: API endpoints (new handlers)
  - `handleMT5SyncAll()` - Sync all integrations
  - `handleMT5SyncIntegration()` - Sync specific integration
  - `syncMT5Integration()` - Internal sync logic

- **`server/index.ts`**: Route registration
  - `POST /api/sync/mt5/trigger` - Trigger all syncs
  - `POST /api/sync/mt5/trigger/:integrationId` - Trigger specific sync

### Frontend

- **`client/lib/mt5.ts`**: MT5 integration management
  - `linkMT5Account()` - Link new account
  - `getMT5Integrations()` - Get all integrations
  - `triggerMT5SyncAll()` - Trigger sync via API
  - `triggerMT5SyncIntegration()` - Trigger single sync
  - `getRecentMT5Syncs()` - Get sync history

- **`client/pages/AdminPage.tsx`**: Admin UI
  - Monitoring tab with MT5 integration section
  - Link MT5 form with platform-agnostic setup
  - Integrations table with sync controls
  - Sync history display

### Database

- **`mt5_integrations` table**: Store MT5 connection details
  - `id` (UUID, primary key)
  - `credential_id` (UUID, foreign key to trading_credentials)
  - `mt5_account_id` (TEXT, MT5 account number)
  - `mt5_api_token` (TEXT, API credential)
  - `mt5_server_endpoint` (TEXT, API endpoint URL)
...
- MT5 API fetch: ~500ms - 2s (varies by provider)
...
3. **Use RLS Policies**: Restrict access to mt5_integrations table
...
```sql
-- Only service role can read/write MT5 integrations
CREATE POLICY "Service role only" ON mt5_integrations
...
2. **No Broker Restrictions**: Works with any broker that supports MT5
...
3. ✅ Link MT5 accounts in Admin → Monitoring
...
5. Review server logs for `[MT5 Sync]` messages
